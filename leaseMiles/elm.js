...
var author$project$Main$init = F3(
	function (flags, url, key) {
		...
		return _Utils_Tuple2(
			A6(author$project$Main$Model, start, author$project$Main$Valid, curr, author$project$Main$Valid, miles, author$project$Main$Valid),
...
var author$project$Main$validate = function (model) {
	var milesPerYearStatus = _Utils_eq(
		elm$core$String$toInt(model.milesPerYear),
		elm$core$Maybe$Nothing) ? author$project$Main$Invalid('Enter a number') : author$project$Main$Valid;
	var errDate = A3(justinmimbs$date$Date$fromCalendarDate, 1980, elm$time$Time$Jan, 1);
	var startDate = function () {
		var _n1 = justinmimbs$date$Date$fromIsoString(model.startDate);
		if (_n1.$ === 'Ok') {
			var dt = _n1.a;
			return dt;
		} else {
	...
	var startDateStatus = _Utils_eq(startDate, errDate) ? author$project$Main$Invalid('Please Enter YYYY-MM-DD') : author$project$Main$Valid;
	var currentDate = function () {
		var _n0 = justinmimbs$date$Date$fromIsoString(model.currentDate);
		if (_n0.$ === 'Ok') {
			var dt = _n0.a;
			return dt;
		} else {
	...
	var currentDateStatus = _Utils_eq(currentDate, errDate) ? author$project$Main$Invalid('Please Enter YYYY-MM-DD') : author$project$Main$Valid;
	return _Utils_update(
		model,
		{currentDateStatus: currentDateStatus, milesPerYearStatus: milesPerYearStatus, startDateStatus: startDateStatus});
};
...
var author$project$Main$CurrentDate = function (a) {
	return {$: 'CurrentDate', a: a};
};
var author$project$Main$MilesPerYear = function (a) {
	return {$: 'MilesPerYear', a: a};
};
var author$project$Main$StartDate = function (a) {
...
var author$project$Main$formatResult = F2(
	function (model, result) {
		var bad = (!_Utils_eq(model.startDateStatus, author$project$Main$Valid)) || ((!_Utils_eq(model.currentDateStatus, author$project$Main$Valid)) || (!_Utils_eq(model.milesPerYearStatus, author$project$Main$Valid)));
		return bad ? A2(elm$html$Html$div, _List_Nil, _List_Nil) : A2(
			elm$html$Html$div,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					elm$html$Html$text(
					A2(cuducos$elm_format_number$FormatNumber$format, cuducos$elm_format_number$FormatNumber$Locales$usLocale, result))
				]));
	});
...
var author$project$Main$view = function (model) {
	var sdate = function () {
		...
		if (_n1.$ === 'Ok') {
			...
			var msg = _n1.a;
			return A3(justinmimbs$date$Date$fromCalendarDate, 1980, elm$time$Time$Jan, 1);
		}
	}();
	var milesPerYear = elm$core$String$toInt(model.milesPerYear);
	var cdate = function () {
		var _n0 = justinmimbs$date$Date$fromIsoString(model.currentDate);
		if (_n0.$ === 'Ok') {
			var dt = _n0.a;
	...
	var daysDiff = A3(justinmimbs$date$Date$diff, justinmimbs$date$Date$Days, sdate, cdate);
	var yearFrac = daysDiff / 365.0;
	var milesShouldBe = A2(elm$core$Maybe$withDefault, 0, milesPerYear) * yearFrac;
	return {
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text('Lease Calculator')
							])),
						A4(author$project$Main$viewInput, 'text', 'Start Date', model.startDate, author$project$Main$StartDate),
						author$project$Main$formatStatus(model.startDateStatus),
						A4(author$project$Main$viewInput, 'text', 'Current Date', model.currentDate, author$project$Main$CurrentDate),
						author$project$Main$formatStatus(model.currentDateStatus),
						A4(author$project$Main$viewInput, 'text', 'Miles Per Year', model.milesPerYear, author$project$Main$MilesPerYear),
						author$project$Main$formatStatus(model.milesPerYearStatus),
